...
class Profiler extends EventEmitter {
	...
	constructor(namespace) {
		...
		this.stats = Profiler.stats[namespace] = {};
		this.events = Profiler.events[namespace] = Array(10);
		this.warnings = Profiler.warnings[namespace] = Array(10);
		this.idCounter = 0;
	...
	emitBegin(block) {
		...
		Profiler.emitter.emit('begin', this.namespace, block);
	...
	emitEnd(block, aggregate) {
		...
		Profiler.emitter.emit('end', this.namespace, block, aggregate);
	...
	emitWarning(warning) {
		...
		Profiler.emitter.emit('warning', this.namespace, warning);
